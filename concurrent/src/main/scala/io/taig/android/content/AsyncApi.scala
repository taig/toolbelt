...
class AsyncApi[T, C]( future: Future[T], context: ⇒ C )( implicit ec: ExecutionContext ) {
    ...
    def onComplete[U]( f: ( C, Try[T] ) ⇒ U ): Unit = future.onComplete( result ⇒ f( context, result ) )( ec )
