...
public class TimePicker extends FrameLayout {
    ...
    // ui components
    private final Number mHourSpinner;

    private final Number mMinuteSpinner;

    private final Number mAmPmSpinner;
    ...
    // version uses a Number spinner. Therefore the code
    ...
    public TimePicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        // hour
        mHourSpinner = (Number) findViewById(R.id.hour);
        mHourSpinner.setOnValueChangedListener(new Number.OnValueChangeListener() {
            public void onValueChange(Number spinner, int oldVal, int newVal) {
                updateInputState();
                if (!is24HourView()) {
        ...
        // minute
        mMinuteSpinner = (Number) findViewById(R.id.minute);
        mMinuteSpinner.setMinValue(0);
        mMinuteSpinner.setMaxValue(59);
        mMinuteSpinner.setOnLongPressUpdateInterval(100);
        mMinuteSpinner.setFormatter(Number.getTwoDigitFormatter());
        mMinuteSpinner.setOnValueChangedListener(new Number.OnValueChangeListener() {
            public void onValueChange(Number spinner, int oldVal, int newVal) {
                updateInputState();
        ...
        if (amPmView instanceof Button) {
            ...
            mAmPmButton = null;
            mAmPmSpinner = (Number) amPmView;
            mAmPmSpinner.setMinValue(0);
            mAmPmSpinner.setMaxValue(1);
            mAmPmSpinner.setDisplayedValues(mAmPmStrings);
            mAmPmSpinner.setOnValueChangedListener(new Number.OnValueChangeListener() {
                public void onValueChange(Number picker, int oldVal, int newVal) {
                    updateInputState();
                    picker.requestFocus();
                    mIsAm = !mIsAm;
                    updateAmPmControl();
    ...
    private void updateHourControl() {
        if (is24HourView()) {
            ...
            mHourSpinner.setFormatter(Number.getTwoDigitFormatter());
        } else {
