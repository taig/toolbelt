...
trait Resource
    ...
    def as[T: ResourceResolver]: T = implicitly[ResourceResolver[T]].resolve( resource )
}

object Resource {
    trait ResourceResolver[T] {
        def resolve( resource: Int )( implicit context: android.content.Context ): T
    }

    implicit val `ResourceResolver[Boolean]` = new ResourceResolver[Boolean] {
        override def resolve( resource: Int )( implicit context: android.content.Context ) = {
