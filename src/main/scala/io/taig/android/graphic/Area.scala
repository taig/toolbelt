...
case class Area[T: Numeric]( position: Point[T], dimension: Dimension[T] )
{
	...
	def corner( a: Direction, b: Direction ): Point[T] = ( a, b ) match
	{
		...
		case ( Top, Right ) | ( Right, Top ) => position.copy( x = position.x + dimension.width )
		case ( Bottom, Right ) | ( Right, Bottom ) => position + dimension
		case ( Bottom, Left ) | ( Left, Bottom ) => position.copy( y = position.y + dimension.height )
	...
	{
		case Left => Area( position, dimension.copy( width = implicitly[Numeric[T]].zero ) )
		case Top => Area( position, dimension.copy( height = implicitly[Numeric[T]].zero ) )
		case Right => Area( position.copy( x = position.x + dimension.width ), dimension.copy( width = implicitly[Numeric[T]].zero ) )
		case Bottom => Area( position.copy( y = position.y + dimension.height ), dimension.copy( height = implicitly[Numeric[T]].zero ) )
	}
