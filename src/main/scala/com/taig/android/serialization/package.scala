...
package object serialization
{
	implicit class RichParcel( parcel: Parcel )
	{
		...
		def readBoolean() =
		{
			parcel.readValue( classOf[Boolean].getClassLoader ).asInstanceOf[Boolean]
		}

		def writeOption[T]( option: Option[T] ) = parcel.writeValue( option.orNull )

		def readOption[T: ClassTag] =
		{
			Option( parcel.readValue( implicitly[ClassTag[T]].runtimeClass.getClassLoader ).asInstanceOf[T] )
		}
