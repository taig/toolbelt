...
object ResourceResolver {
  def apply[A, B](implicit rr: ResourceResolver[A, B]): ResourceResolver[A, B] =
    rr
  ...
  implicit val resourceResolver_Res_CharSequence
    : ResourceResolver[Int, CharSequence] = {
    instance { (context, resource, _) ⇒
  ...
  implicit val resourceResolver_Res_Drawable
    : ResourceResolver[Int, Drawable] = {
    instance { (context, resource, _) ⇒
  ...
  implicit val resourceResolver_Res_Int: ResourceResolver[Int, Int] = {
    instance { (context, resource, _) ⇒
      context.getResources.getResourceTypeName(resource) match {
        ...
        case name ⇒
          sys.error(s"Invalid resource type $name given to resolve an Int")
      }
    }
  }

  implicit val resourceResolver_Res_ArrayInt
    : ResourceResolver[Int, Array[Int]] = {
    instance { (context, resource, _) ⇒
  ...
  implicit val resourceResolver_Res_ArrayCharSequence
    : ResourceResolver[Int, Array[CharSequence]] = {
    instance { (context, resource, _) ⇒
      context.getResources.getTextArray(resource)
    }
  }

  implicit val resourceResolver_Res_ArrayString
    : ResourceResolver[Int, Array[String]] = {
    instance { (context, resource, _) ⇒
  ...
  implicit val resourceResolver_Quantity_String
    : ResourceResolver[Quantity, String] = {
