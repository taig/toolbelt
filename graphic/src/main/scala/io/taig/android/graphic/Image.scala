...
import io.taig.android.graphic.syntax.dimension._
import io.taig.android.graphic.syntax.numeric._
...
class Image private (stream: ⇒ InputStream) {
  ...
  def this(uri: Uri)(implicit context: Context) =
  ...
  require(resolution.width > -1 && resolution.height > -1,
  ...
  def decode(target: Dimension[Int], matrix: Matrix): Bitmap =
    decode(target, matrix, defaultOptions)

  def decode(target: Dimension[Int], options: Options): Bitmap =
    decode(target, null: Matrix, options)

  def decode(target: Dimension[Int],
             matrix: Matrix,
  ...
  def decode(matrix: Matrix, options: Options): Bitmap =
    decode(1, matrix, options)

  def decode(scale: Float): Bitmap = decode(scale, defaultOptions)

  def decode(scale: Float, matrix: Matrix): Bitmap =
    decode(scale, matrix, defaultOptions)

  def decode(scale: Float, options: Options): Bitmap =
    decode(scale, null: Matrix, options)
  ...
  def decode(clipping: Area[Int], matrix: Matrix): Bitmap =
    decode(clipping, matrix, defaultOptions)

  def decode(clipping: Area[Int], options: Options): Bitmap =
    decode(1, clipping, options)

  def decode(clipping: Area[Int], matrix: Matrix, options: Options): Bitmap = {
    decode(1, clipping, matrix, options)
  }

  def decode(scale: Float, clipping: Area[Int]): Bitmap =
    decode(scale, clipping, defaultOptions)
        ...
        try {
          createBitmap(
            ...
            Math.min(
              ...
              m.postScale(options.inSampleSize * scale,
