...
final class view(view: View) {
  ...
  /**
    ...
    */
  def setPadding(padding: Int) =
    view.setPadding(padding, padding, padding, padding)

  /**
    * Apply vertical and horizontal padding
    */
  def setPadding(vertical: Int, horizontal: Int) =
    view.setPadding(horizontal, vertical, horizontal, vertical)

  def setPadding(padding: Distance[Int]) =
  ...
  def next: Option[View] = view match {
    case viewGroup: android.view.ViewGroup if viewGroup.getChildCount > 0 ⇒
      Some(viewGroup.getChildAt(0))
    case view ⇒
