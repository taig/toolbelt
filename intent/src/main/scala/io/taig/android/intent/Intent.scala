package io.taig.android.intent
...
object Intent {
    ...
    def apply( `class`: Class[_] )( implicit c: Context ): Intent = new Intent( c, `class` )

    def apply[T]( implicit c: Context, tag: ClassTag[T] ): Intent = {
        new Intent( c, implicitly[ClassTag[T]].runtimeClass )
    }

    def apply( action: String, uri: Uri, `class`: Class[_] )( implicit c: Context ): Intent = {
